...
import { createMemo, createSignal, Errored, Loading, refresh } from "solid-js";
...
const AccountFilter = (properties: { accountId: string | undefined; onChoose: (accountId: string | undefined) => void; }) => {
  const options = createMemo(async () => {
    ...
    return [
      ...
      ...accounts.map(account => ({ value: account.account_id, label: accountText(account) })),
    ];
  });
...
export const RequestsPage = () => {
  ...
  const rows = createMemo(() => {
    ...
    if (cursors().length > 0) return { events: seed.events, olderCursor: seed.next_before };
    ...
    const events = combined.slice(0, MAX_LIVE_ROWS);
  ...
  return (
    <div class="space-y-6">
      ...
      <Errored fallback={(error, reset) => <RegionFailure error={error()} retry={reset} />}>
        <Loading fallback={<RegionPending label="Loading requests" />}>
          <div class="space-y-3">
            <EventsTable events={rows().events} />
